...
BEGIN
  ...
  DECLARE v_new_value_id INT UNSIGNED;
	...
	ELSE
		...
		IF p_file_id_array IS NOT NULL THEN CALL _set_file_to_value_attachments(v_value_id, p_file_id_array, 1); END IF;
...
BEGIN
	...
	FOR v_unit IN (SELECT id FROM course_design_units WHERE subsection_id = (SELECT subsection_id FROM course_design_units WHERE id = p_movable_unit_id) ORDER BY -index_number DESC) DO
	
		IF v_unit.id = p_front_unit_id THEN
			UPDATE course_design_units SET index_number = v_counter WHERE id = p_movable_unit_id;
			SET v_counter = v_counter + 1;
		END IF;
		
		IF v_unit.id != p_movable_unit_id THEN
			UPDATE course_design_units SET index_number = v_counter WHERE id = v_unit.id;
			SET v_counter = v_counter + 1;
		END IF;
...
BEGIN
	...
	DECLARE v_entry_id INT UNSIGNED;
	DECLARE v_index_number INT UNSIGNED DEFAULT 0;
	DECLARE v_entry_indexes TEXT DEFAULT '{}';
  ...
  SET v_entry_id_array = CONCAT('[', v_entry_id_array, ']');
	...
	FOR _index IN 0 .. JSON_LENGTH(v_entry_id_array) - 1 DO
		SET v_entry_id = JSON_VALUE(v_entry_id_array, CONCAT('$[', _index, ']'));
		IF v_entry_id = p_put_before_entry_id THEN
			JSON_INSERT(v_entry_indexes, CONCAT('$.', p_moving_entry_id), v_index_number));
			SET v_index_number = v_index_number + 1;
		END IF;
		IF v_entry_id != p_moving_entry_id THEN
			JSON_INSERT(v_entry_indexes, CONCAT('$.', v_entry_id), v_index_number));
			SET v_index_number = v_index_number + 1;
		END IF;
	END FOR;
	IF p_put_before_entry_id IS NULL THEN
		JSON_INSERT(v_entry_indexes, CONCAT('$.', p_moving_entry_id), index_number));
	END IF;
