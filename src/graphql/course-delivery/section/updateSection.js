...
export default {
  ...
  async resolve(obj, { id, ...inputArgs }, { user, db }) {
    ...
    try {
      ...
      const assignmentList = await sqlBuilder.buildAssignmentList(inputArgs, { id });
      if (assignmentList === '') return 0;
      const { affectedRows } = await db.query(`UPDATE course_delivery_sections SET ${assignmentList} WHERE id = ${id}`);
      return affectedRows;
